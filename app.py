...
# ── Database ──────────────────────────────────────────────────
def get_db():
    db = sqlite3.connect(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'instance', 'database.db'))
    db.row_factory = sqlite3.Row
    return db

def init_db():
    with app.app_context():
        # Force create instance folder
...
def get_confidence(text):
    ...
    decision    = proba_model.named_steps['svm'].decision_function(
        proba_model.named_steps['tfidf'].transform([text])
    )
    import numpy as np
    scores     = decision[0]
